...
import {GET_COLLECTIONS, setCollections, setLoading} from '../redux/actions/collectionsActions'
...
const resource = async (method = '', params = {}) => {
  const {error, data} = await new Api().collections[method](params);
 ...
 */
function* setCollectionsWorker(action = {}) {
  const params = action?.payload;
  try {
    yield put(setLoading());
    const collections = yield call(resource, 'list', params);
...
export function* collectionsWatcher() {
  yield takeEvery(GET_COLLECTIONS, setCollectionsWorker)
}
